...
class GenDomsController {
    ...
    def index() { 
		...
		//Einlesen der Axiome in eine Liste,
		//auf diese werden die Umsetzungsregeln (i)...(v) angewandt 
		...
		//Die KlassenList enhält nach Anwendung der Umsetzungsregeln (i)...(v)
		...
		klassen.each {
			Map m = it
			String p = m.Paketname
			p = p.replace('.', '/')
			String pfadName = "./grails-app/domain/"+p+'/'
			BufferedWriter d = groovyFile(pfadName+m.Klasse+".groovy")
			...
			if (istOberklasse (m.Klasse.toString(), klassen)) {
				...
				//d.writeLine('String toString() {"${this.name}" }')
			}
			if (m.hasMany) {
				Map hasManyMap = [:]
				m.hasMany.each {
		...
		BufferedWriter gvIn = groovyFile("./"+f.getOriginalFilename().replace(".owl", ".gv"))
